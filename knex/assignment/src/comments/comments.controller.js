...
async function listCommenterCount(req, res, next) {
  let userCommentCounts = await service.listCommenterCount();
  userCommentCounts = userCommentCounts.map((comment) => ({
    ...comment,
    count: parseInt(comment.count, 10),
  }));
  res.json({ data: userCommentCounts });
}

class BinarySearchTree {
  ...
  countLeaves(node = this) {
    let count = 0;
    if (node.left) {
      count += this.countLeaves(node.left); //> return 1
    }
    if (node.right) {
      count += this.countLeaves(node.right); //> 1
    }
  ...
  isBalancedBST(node = this) {
    if (!node) {
      return 0;
    }
    const height = node.getHeight();
    if (node.isBalancedBST(node.left) <= 1 && node.isBalancedBST(node.right) <= 1) {
      return height;
    } else {
      return -1;
    }

...
function renderContact(contact) {
  ...
  const { id, picture, name, email, phone, website, address, company } = contact;
  return `
...
function filterByCity(city) {
  return contacts.filter((selected) => selected.address.city == city);
}
...
function filterHandler(event) {
  ...
  render(contacts);
  if (value == "0") render(contacts);
  if (value != "0") {
    const contactsFiltered = filterByCity(value);
    render(contactsFiltered);
  }
}

/*
  Accepts an array of contacts.
  Populate the select element with the id `filterOptions` with the list of cities.
...
function loadCities(contacts) {
  const mapAllCities = contacts.map((selected) => selected.address.city);

  const obj = {};
  // SHORTHAND???
  // for (let selected of mapAllCities) obj[selected] = 1;
  for (let selected of mapAllCities) obj[selected] = 1;

  const cities = Object.keys(obj);

  // TRY TO FIX ZERO CITIES TEST HERE WITH IF CONDITIONS
  const filterOptions = document.querySelector("#filterOptions");
  for (let selected of cities) {
    const cityOption = document.createElement("option");
    cityOption.setAttribute("value", selected);
    cityOption.innerText = selected;
    filterOptions.appendChild(cityOption);
  }
  if (cities.length < 5) {
    // get the current options
    const options = document.querySelectorAll("option");
    // get the current cities > cities
    for (let selected of options) {
      if (
        !cities.includes(selected.innerText) &&
        !selected.innerText.includes("city")
      ) {
        filterOptions.removeChild(selected);
      }
    }
  }

  // make the elements and push them
}
...
function deleteContact(id) {
  contacts = contacts.filter((selected) => selected.id != id);
  return contacts;
}
...
function main() {
  ...
  main.addEventListener("click", (event) => {
    if (event.target.nodeName == "BUTTON") deleteButtonHandler(event);
  });
